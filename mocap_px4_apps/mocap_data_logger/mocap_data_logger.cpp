...
float roll_custom_radian = 0;
float pitch_custom_radian = 0;
float yaw_custom_radian = 0;

float roll_custom_degrees = 0;
float yaw_custom_degrees = 0;
...
// Sleep a little so that we don't lumber the CPU with a busy poll
      ...
      for (unsigned int SubjectIndex = 0; SubjectIndex < SubjectCount;
        ...
        for (unsigned int SegmentIndex = 0; SegmentIndex < SegmentCount;
          ...
          // roll_custom = 180_degress - roll;
          roll_custom_radian = M_PI - euler_orientation(0);
          pitch_custom_radian = -euler_orientation(1);
          yaw_custom_radian = euler_orientation(2) + yaw_offset_radians;

          if (euler_orientation(0) > 0)
            roll_custom_degrees = 180 - (euler_orientation(0) * (180 / M_PI));

          else if (euler_orientation(0) < 0)
            roll_custom_degrees = -180 - (euler_orientation(0) * (180 / M_PI));

          if (euler_orientation(2) > 0)
            yaw_custom_degrees = 180 - (euler_orientation(2) * (180 / M_PI));
          ...
          std::cout << '\t' << "Euler Orientation degrees: "
                    ...
                    << -euler_orientation(1) * (180 / M_PI) << '\t' << "yaw "
                    << yaw_custom_degrees << '\n';
